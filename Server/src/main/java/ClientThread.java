    ...
    private void execute(Packet receivedPacket) {
        ...
        switch (receivedPacket.getCommand()) {
            ...
            case REGISTER -> {
                ...
                UserManagement.INSTANCE.register(receivedPacket.getUser());
                responsePacket = Packet.builder().message("User registered").build();
            }
            case MESSAGE_ALL -> {
            ...
            case MESSAGE_ROOM -> {
                // Handle room messaging (not implemented)
                UserManagement.INSTANCE.roomMessage(receivedPacket);
            }
            case ENTER_ROOM -> {
                // Handle entering a room (not implemented)
